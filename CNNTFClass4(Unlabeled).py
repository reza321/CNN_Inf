...
random_sample = np.random.choice(len(X_unlabeled), size=round(unlabeled_prop* len(X_unlabeled)), replace=False, p=None)
X_unlabeled_prop = X_unlabeled[random_sample] #shape=(unlabeled_prop* len(X_unlabeled),1,248,4)
print()
...
loss_cls = tf.reduce_mean(cross_entropy,name='loss_cls')

# loss_cls_per_label = tf.reduce_mean(classifier_output,name='loss_cls_per_label',axis=0)
...
# grad_per_label_loss_op = tf.gradients(loss_cls_per_label, params)
grad_loss_no_reg_op =tf.gradients(total_loss,params)
...
def test_retraining(test_idx, iter_to_load=0, force_refresh=False, 
                    num_to_remove=5, num_steps=num_steps, random_seed=0,remove_type='maxinf'):
    np.random.seed(0)
    # load_checkpoint(0)
    ...
    predicted_loss_diffs=get_influence_on_test_loss(
        [test_idx],
        np.arange(len(X_unlabeled_prop)))
...
def get_influence_on_test_loss(test_indices,unlabeled_train_idx,approx_type='cg'):
    ...
    num_to_remove=len(unlabeled_train_idx)
    predicted_loss_diffs=np.zeros([num_to_remove])    
    influence_function_val=np.zeros((len(X_unlabeled_prop),num_class))
    predicted_labels=np.zeros((len(X_unlabeled_prop),num_class))
    # print(np.shape(influence_function_val))
    # print(np.shape(X_unlabeled_prop))
    # print(np.shape(Train_Y))
    # print(Train_Y[0])
    # print(np.shape(Train_X))    
    # exit()

    for counter,idx_to_remove in enumerate(unlabeled_train_idx):
        ...
        temp_input_feed=np.expand_dims(X_unlabeled_prop[idx_to_remove,:], axis=0)   #(1,1,248,4)
        for label in range(0,num_class):
            labels=[0.]*(num_class)            
            labels[label]=1.0
            temp_labels_feed=np.array(labels).reshape(1,5)               #(1,5)        
            ...
            influence_function_val[counter][label]=np.dot(np.reshape(inverse_hvp,-1), train_grad_loss_val2) / num_train_examples
            predicted_labels[counter]=[0 if i!=influence_function_val.argmax() else 1 for i in range(num_class)]

        print(influence_function_val[counter])
        print(predicted_labels[counter])

        predicted_label=influence_function_val.absolute(influence_function_val).max(axis=1)        
        prediction_loss_diffs=np.absolute(influence_function_val).max(axis=1)
        print(prediction_loss_diffs[counter])
        print(np.shape((prediction_loss_diffs)))
        exit()
