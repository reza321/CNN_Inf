...
def training(one_fold, X_unlabeled, seed, prop, num_filter_ae_cls_all, epochs_ae_cls=20):
    ...
    random.seed(seed)
    np.random.seed(seed)
    random_sample = np.random.choice(len(Train_X), size=round(0.9*len(Train_X)), replace=False, p=None)
    ...
    Train_X_Comb = X_unlabeled   #    shpae (10876,1,248,4)
    ...
    for z in range(len(num_filter_ae_cls_all)):
        ...
        with tf.Session() as sess:
            ...
            for k in range(epochs_ae_cls):
                ...
                # print(np.shape(x_combined_index)) #shape=(7251,)
                # print(np.shape(x_labeled_index)) #shape=(7251,)
...
test_accuracy_fold, test_metrics_fold, mean_std_acc, mean_std_metrics = training_all_folds(
    label_proportions=[0.1], num_filter=[32, 32, 64, 64])
