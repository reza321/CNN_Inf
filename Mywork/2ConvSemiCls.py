...
def classifier_mlp(latent_dim,num_filter_ae_cls,input_combined, input_labeled, num_class, num_filter_cls, num_dense):
...
def semi_supervised(input_labeled, input_combined, true_label, alpha, beta, num_class, latent_dim, num_filter_ae_cls, num_filter_cls, num_dense, input_size):
    ...
    classifier_output, dense = classifier_mlp(latent_dim=latent_dim, num_filter_ae_cls=num_filter_ae_cls,
                                                input_combined=input_combined, input_labeled=input_labeled,
                                                num_class=num_class, num_filter_cls=num_filter_cls, num_dense=num_dense)

    # BECAUSE OF "latenet_labeled" ENCODER is Common in both cls and AE.
    ...
    # return classifier_output,loss_ae, loss_cls, accuracy_cls, train_op_ae, train_op_cls, classifier_output, dense, train_op, total_loss
    return classifier_output, loss_cls, accuracy_cls, train_op_cls, classifier_output, dense, train_op, total_loss
...
def training(one_fold, X_unlabeled, seed, prop, num_filter_ae_cls_all, epochs_ae_cls=20):
    ...
    # print(np.shape(Train_X)) #shape--->(19287,1,248,4)
    # print(np.shape(Train_Y_ori)) #shape--->(19287)
    random.seed(seed)
    np.random.seed(seed)
    random_sample = np.random.choice(len(Train_X), size=round(0.05*len(Train_X)), replace=False, p=None)
    Train_X = Train_X[random_sample]
    Train_Y_ori = Train_Y_ori[random_sample]
    # print(np.shape(Train_X))     #shape--->(964,1,248,4)
    # print(np.shape(Train_Y_ori)) #shape--->(964)
    # exit() 
    ...
    random_sample = np.random.choice(len(X_unlabeled), size=round(0.05 * len(X_unlabeled)), replace=False, p=None)
    X_unlabeled = X_unlabeled[random_sample]
    Train_X_Comb = X_unlabeled
    # print(np.shape(Train_X_Comb))     #shape--->(3625,1,248,4)
    ...
    for z in range(len(num_filter_ae_cls_all)):
        ...
        with tf.Session() as sess:
            ...
            # DEFINES ALL THE TENSORS NEEDED FOR THE CALCULAITON.
            classifier_output, loss_cls, accuracy_cls, train_op_cls, classifier_output, dense,train_op, total_loss= semi_supervised(
            ...
            # classifier_output,loss_ae, loss_cls, accuracy_cls, train_op_ae, train_op_cls, classifier_output, dense,train_op, total_loss= semi_supervised(
            #     input_labeled=input_labeled, input_combined=input_combined, true_label=true_label, alpha=alpha,
            #     beta=beta, num_class=num_class, latent_dim=latent_dim, num_filter_ae_cls=num_filter_ae_cls,
            #     num_filter_cls=num_filter_cls, num_dense=num_dense, input_size=input_size)            
            ...
            for k in range(1):
                ...
                x_labeled_index = get_labeled_index(train_x_comb=Train_X_Comb, train_x=Train_X)
                # print(np.shape(x_combined_index))      # shape --->(3625,)
                # print(np.shape(x_labeled_index))       # shape --->(3625,)
                ...
                #     # classifier_output,loss_ae_, loss_cls_, accuracy_cls_, _ = sess.run([classifier_output,loss_ae, loss_cls, accuracy_cls, train_op],
                #     #                                                  feed_dict={alpha: alfa_val, beta: beta_val,
                #     #                                                             input_combined: X_ae,
                #     #                                                             input_labeled: X_cls,
                #     #                                                             true_label: Y_cls})
                #     classifier_output, loss_cls_, accuracy_cls_, _ = sess.run([classifier_output, loss_cls, accuracy_cls, train_op],
                #                                                      feed_dict={alpha: alfa_val, beta: beta_val,
                #                                                                 input_combined: X_ae,
                #                                                                 input_labeled: X_cls,
                #                                                                 true_label: Y_cls})
                ...
                X_ae = Train_X_Comb[unlab_index_range]
                X_cls = Train_X[x_labeled_index]
                Y_cls = Train_Y[x_labeled_index]
                
                # Az khodam dar ovordam
                loss_ae_=0
                ...
                print('====================================================')
                if not os.path.exists("Conv-Semi-TF-PS/" + '2/' + str(z) + '/' + str(prop)):
                    os.makedirs("Conv-Semi-TF-PS/" + '2/' + str(z) + '/' + str(prop))
...
test_accuracy_fold, test_metrics_fold, mean_std_acc, mean_std_metrics = training_all_folds(
    label_proportions=[0.05, 1], num_filter=[32, 32, 64, 64])
